...
//
// 완성본은 out/template.html 로 나온다. 만들어 낸 것은 모두 out/ 아래 모이고
// 그 폴더만 통째로 .gitignore 한다. 소스는 27KB 뿐이고 나머지는 전부 여기서
// 만들어 붙인다 — 글꼴, 팝업 UI, 창 안 화면.
...
const OUT = resolve(ROOT, 'out', 'template.html');
...
const popup = Object.fromEntries(['ko', 'en'].map((l) => [l, Object.fromEntries(
  ['idle', 'done'].map((s) => [s, `data:image/png;base64,${b64(resolve(ROOT, `out/ui/ui-${l}-light-${s}.png`))}`]))]));
...
console.log(`out/template.html ${(Buffer.byteLength(html) / 1024 / 1024).toFixed(2)} MB — 크롬에서 열면 된다`);
